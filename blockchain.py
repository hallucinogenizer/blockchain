...
class Blockchain():
    ...
    def add_block(self, block):
        ...
        if len(self.blocks)>1:
            block.hash_previous_block = hash_sha256(str(self.blocks[-1]).encode())
        
        
        for i in range(len(block.delivery.created_msgs)):
            block.delivery.created_msgs[i].id=MsgId(i,self.count)
    ...
    def check_blockchain(self):
        ...
        if len(self.blocks)<1:
            ...
            for i in range(len(self.blocks)):
                if (i<len(self.blocks)-1 and hash_sha256(str(self.blocks[i]).encode())!=self.blocks[i+1].hash_previous_block):
                    all_good=False
